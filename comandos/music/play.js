...
const apikey = "AIzaSyDFy0dUuHbGtKIooXRcuzPc-AVuVJMcSpk"
        ...
        if(message.guild.me.voiceChannel && message.guild.me.voiceChannelID!=message.member.voiceChannelID)
            ...
            try {
                ...
                if(!args[0].match(check)){
                    ...
                    let escolhida=null;
                    let embed= new Discord.MessageEmbed().setTitle('Escolha de 1 a 5')
                    ...
                    message.channel.send(embed).then(async (msg)=>{
                        ...
                        collector.on('collect', async (e)=>{
                            ...
                            //verifica se o bot está em conectado, se não, ele será conectado
                            if(!data.connection)
                                data.connection = message.member.voice.channel.join()
                            //se não ouver nenhum trasnmissão, será chamada a função de tocar
                            if(!data.dispatcher)
                                await play(client,opts,data);
                            else{
...
async function play(client,opts,data){
    ...
    if(data.dispatcher){
        let embed = new Discord.MessageEmbed().addField("Música:",'['+data.queue[0].nome+']('+data.queue[0].url+')').addField("Canal:",data.queue[0].canal)
